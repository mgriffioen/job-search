...
export function scoreJob(job, profile, now = new Date()) {
  ...
  const raw = capabilities.score + title.score + skills.score + context.score + penalties.score;
  ...
  const reasons = [...capabilities.reasons, ...title.reasons, ...skills.reasons, ...context.reasons, ...penalties.reasons]
